...
const App: React.FC = () => {
  ...
  const stats: DashboardStats = React.useMemo(() => {
    if (!userSettings) return {
        ...
        dailyLimit: 0, daysRemaining: 0, cycleStartDate: '', cycleEndDate: ''
    };
    ...
    <div className="flex h-screen bg-slate-950 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0 md:pl-64 transition-all duration-300">
        ...
        <header className="md:hidden flex items-center justify-between p-4 bg-slate-900 border-b border-slate-800 sticky top-0 z-30">
          <div className="flex items-center gap-2">
             <div className="w-8 h-8 bg-indigo-600 rounded-lg flex items-center justify-center text-white font-bold">FA</div>
             <span className="font-bold text-white">Finans Asistanı</span>
          </div>
          ...
          <div className="hidden md:flex justify-between items-center mb-8">
             <div>
                <h2 className="text-2xl font-bold text-white capitalize">
                  {activeTab === 'ai' ? 'AI Finans Uzmanı' : 
                   activeTab === 'calendar' ? 'Takvim Yönetimi' :
                   activeTab === 'settings' ? 'Ayarlar' : 
                   activeTab === 'dashboard' ? 'Genel Bakış' : 
                   activeTab === 'history' ? 'Dönem Geçmişi' : 'İşlem Listesi'}
                </h2>
                <p className="text-slate-400 text-sm">
                  Hoş geldin, finansal hedeflerine bugün bir adım daha yaklaş.
                </p>
             </div>
             <div className="flex items-center gap-4">
                {userSettings && activeTab !== 'settings' && (
                ...
                <button
                    onClick={() => { setFormInitialDate(undefined); setIsFormOpen(true); }}
            ...
            {activeTab === 'settings' && (
                <BudgetPlanner 
                    ...
                    onSave={(newSettings) => {
                        ...
                        // Kullanıcıyı burada tutuyoruz, dashboard'a atmıyoruz ki değişikliği görsün.
            ...
            {activeTab === 'dashboard' && userSettings && (
              <div className="animate-fade-in space-y-6">
                ...
                <div className="bg-gradient-to-r from-indigo-900/40 to-purple-900/40 border border-indigo-500/20 p-4 md:p-6 rounded-2xl flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        <p className="text-indigo-200 text-sm font-medium mb-1">Aktif Dönem: <span className="text-white font-bold">{stats.cycleStartDate} - {stats.cycleEndDate}</span></p>
      ...
      {isFormOpen && (
        <TransactionForm 
          ...
          onClose={() => { setIsFormOpen(false); setFormInitialDate(undefined); }}
