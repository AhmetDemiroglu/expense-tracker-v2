...
export const calculateHistorySummaries = (transactions: Transaction[], settings: UserSettings): CycleSummary[] => {
    const summaries: CycleSummary[] = [];
    if (transactions.length === 0) return [];

    // Fallback logic: Generate history based on current settings anchor
    // Note: Ideally, we should calculate this based on stored BudgetPeriods, 
    // but for now we use the generated logic based on the user's manual dates if periods aren't fully historical.
    
    const currentStart = new Date(settings.periodStartDate);
    const currentEnd = new Date(settings.periodEndDate);
    
    // Calculate length of the cycle in ms
    const cycleDuration = currentEnd.getTime() - currentStart.getTime();
    
    // Iterate backwards
    let pointerEnd = new Date(currentStart.getTime() - (24*60*60*1000)); // One day before start
    
    let loopGuard = 0;
    const oldestTx = transactions[transactions.length - 1]; // Assuming sorted desc
    const oldestDate = oldestTx ? new Date(oldestTx.date) : new Date();

    while (pointerEnd >= oldestDate && loopGuard < 24) {
        loopGuard++;
        const pointerStart = new Date(pointerEnd.getTime() - cycleDuration);
        
        const cycleTxs = transactions.filter(t => {
            const tDate = new Date(t.date);
            return tDate >= pointerStart && tDate <= pointerEnd;
        });

        if (cycleTxs.length > 0) {
             const inc = cycleTxs.filter(t => t.type === 'income').reduce((a, b) => a + b.amount, 0);
             const exp = cycleTxs.filter(t => t.type === 'expense').reduce((a, b) => a + b.amount, 0);
             
             const totalInc = inc + settings.monthlyIncome;
             const totalExp = exp + settings.fixedExpenses;
             const balance = totalInc - totalExp;
             const savingsRate = totalInc > 0 ? (balance / totalInc) * 100 : 0;

             summaries.push({
                 id: pointerStart.toISOString(),
                 startDate: pointerStart.toLocaleDateString('tr-TR'),
                 endDate: pointerEnd.toLocaleDateString('tr-TR'),
        ...
        // Move pointer back
        pointerEnd = new Date(pointerStart.getTime() - (24*60*60*1000));
    }
