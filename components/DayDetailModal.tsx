    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-fade-in">
        <div className="bg-slate-900 w-full max-w-lg rounded-2xl border border-slate-700 shadow-2xl overflow-hidden flex flex-col max-h-[85vh]">
            ...
            <div className={`p-6 border-b flex justify-between items-center ${
                data.status === 'success' ? 'bg-emerald-900/20 border-emerald-900/50' :
                data.status === 'warning' ? 'bg-amber-900/20 border-amber-900/50' :
                data.status === 'danger' ? 'bg-rose-900/20 border-rose-900/50' : 'bg-slate-800 border-slate-700'
            }`}>
                <div>
            ...
            <div className="p-6 grid grid-cols-2 gap-4 bg-slate-800/50">
                <div className={`p-4 rounded-xl border ${isNegativeLimit ? 'bg-rose-950/30 border-rose-900/50' : 'bg-slate-800 border-slate-700'}`}>
                    <p className={`text-xs font-bold uppercase mb-1 ${isNegativeLimit ? 'text-rose-400' : 'text-slate-500'}`}>
                        {isNegativeLimit ? 'Açık (Borç)' : 'Günlük Limit'}
                    </p>
                    <p className={`text-2xl font-bold ${isNegativeLimit ? 'text-rose-500' : 'text-white'}`}>
                        {Math.round(data.limit).toLocaleString('tr-TR')} ₺
                    </p>
                </div>
                <div className="p-4 rounded-xl bg-slate-800 border border-slate-700">
                    <p className="text-xs font-bold text-slate-500 uppercase mb-1">Gerçekleşen Harcama</p>
                    <p className={`text-2xl font-bold ${data.spent > 0 ? 'text-white' : 'text-slate-400'}`}>
                        {data.spent.toLocaleString('tr-TR')} ₺
                    </p>
                </div>
            </div>
            ...
            <div className="px-6 pb-2">
                <div className={`w-full p-3 rounded-lg flex items-center gap-3 text-sm font-medium ${
                     data.status === 'success' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' :
                     data.status === 'warning' ? 'bg-amber-500/10 text-amber-400 border border-amber-500/20' :
                     data.status === 'danger' ? 'bg-rose-500/10 text-rose-400 border border-rose-500/20' : 'bg-slate-800 text-slate-400'
                }`}>
                    {data.status === 'success' && (
                        <>
                            <span className="text-lg">🎉</span>
                            <span>Harika! Limitin altında kalarak <strong>{Math.round(variance).toLocaleString()} ₺</strong> tasarruf ettin.</span>
                        </>
                    )}
                    {data.status === 'warning' && (
                        <>
                    ...
                    {data.status === 'danger' && (
                        <>
                            <span className="text-lg">🚨</span>
                            <span>Limit aşıldı! Bütçeden <strong>{Math.abs(Math.round(variance)).toLocaleString()} ₺</strong> fazla harcadın.</span>
                        </>
                    )}
                     {data.status === 'neutral' && (
