...
export const BudgetPlanner: React.FC<BudgetPlannerProps> = ({ userId, currentSettings, onSave }) => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
        ...
        // 1. SADECE Listeye/Veritabanına Kaydet
  ...
  const handleActivate = async (p: BudgetPeriod) => {
      // Loading state buton üzerinde gösterilecek
      setLoading(true);
      try {
          ...
          // Listeyi tekrar yükle ki UI (Active Badge) güncellensin
          // (Gerekli olmayabilir ama garanti olsun)
          await loadPeriods();

      } catch (error) {
          console.error("Aktifleştirme hatası:", error);
          alert("Aktifleştirme başarısız oldu.");
      } finally {
          setLoading(false);
      }
  };
  ...
  const handleDelete = async (id: string) => {
      if(window.confirm('Bu dönem kaydını listeden silmek istediğinize emin misiniz?')) {
        ...
        <div className="flex items-center gap-3 mb-6 border-b border-slate-700 pb-4">
            ...
            <div>
                <h2 className="text-xl font-bold text-white">
                    {editingId ? 'Dönemi Düzenle' : 'Yeni Dönem Oluştur'}
                </h2>
                <p className="text-sm text-slate-400">
                    {editingId ? 'Seçili dönemin bilgilerini güncelliyorsunuz.' : 'Önce dönemi oluşturun, sonra aşağıdaki listeden seçerek aktif edin.'}
                </p>
            </div>
            {editingId && (
        ...
        <form onSubmit={handleSave} className="space-y-6">
            ...
            <div className="bg-slate-900/50 p-4 rounded-xl border border-slate-700/50 grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="md:col-span-2 flex items-center gap-2 text-xs text-indigo-300">
                    ...
                    <span>Bu dönem toplam <strong>{isNaN(duration) ? 0 : duration} Gün</strong> sürecektir.</span>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="number" value={income} onChange={(e) => setIncome(e.target.value)} className="w-full bg-slate-900 border border-slate-600 rounded-lg px-3 py-3 text-white focus:ring-2 focus:ring-indigo-500 outline-none" placeholder="0.00" />
                </div>
                <div>
                    <label className="block text-xs font-medium text-slate-400 mb-2">Sabit Giderler Toplamı</label>
                    <input type="number" value={fixedExpenses} onChange={(e) => setFixedExpenses(e.target.value)} className="w-full bg-slate-900 border border-slate-600 rounded-lg px-3 py-3 text-white focus:ring-2 focus:ring-rose-500 outline-none" placeholder="0.00" />
                </div>
            </div>
          ...
          {periods.length === 0 ? (
              ...
              <div className="grid grid-cols-1 gap-4">
                  {periods.map(p => {
                      ...
                      const isActive = currentSettings?.periodName === p.name && 
                                     currentSettings?.periodStartDate === p.startDate;
                      ...
                      return (
                          <div key={p.id} className={`relative p-5 rounded-xl border transition-all ${isActive ? 'bg-slate-800 border-emerald-500 ring-1 ring-emerald-500/50 shadow-lg shadow-emerald-900/20' : 'bg-slate-800/50 border-slate-700 hover:bg-slate-800 hover:border-slate-600'}`}>
                              {isActive && (
                              ...
                              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                                  <div className="flex-1">
                                      ...
                                      <div className="flex items-center gap-3 text-sm text-slate-400 mt-2">
                                          <span className="bg-slate-900 px-2 py-1 rounded border border-slate-700">📅 {p.startDate} ➜ {p.endDate}</span>
