...
export const CalendarView: React.FC<CalendarViewProps> = ({ 
  ...
  const getDayData = (day: Date): DailyStatus => {
    ...
    const isWithinCycle = dayStart.getTime() >= startOfDay(cycleStart).getTime() && 
                          dayStart.getTime() <= startOfDay(cycleEnd).getTime();
    ...
    const cycleIncomeTransactions = transactions.filter(t => {
        ...
        return t.type === 'income' && 
               startOfDay(tDate).getTime() >= startOfDay(cycleStart).getTime() && 
               startOfDay(tDate).getTime() <= startOfDay(cycleEnd).getTime();
    });
    ...
    const cycleTransactionsBeforeToday = transactions.filter(t => {
        ...
        return t.type === 'expense' && 
               startOfDay(tDate).getTime() >= startOfDay(cycleStart).getTime() && 
               startOfDay(tDate).getTime() < dayStart.getTime();
    });
      ...
      <div className="flex items-center justify-between bg-slate-800 p-4 rounded-2xl border border-slate-700">
        ...
        <div className="text-center">
            ...
            <p className="text-xs text-slate-500">
                {settings.periodName ? settings.periodName : 'Varsayılan Dönem'}
            </p>
        </div>
        <button 
                                ...
                                <>
                                    <div className="flex justify-between items-end">
                                        ...
                                        <span className={clsx(
                                            ...
                                            isNegativeLimit ? "text-rose-400" : "text-slate-300"
                                        )}>
                                        ...
                                        {isNegativeLimit ? (
                                            ...
                                            <div 
                                                className={clsx("h-full transition-all duration-500", 
                                                    ...
                                                    data.status === 'danger' ? 'bg-rose-500' : 'bg-slate-600'
                                                )}
